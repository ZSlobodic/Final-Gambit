    ...
    <tbody>
        @foreach ($feats as $feat)
            <tr>
                ...
                <td><a class="btn d-grid gap-2 btn-primary btn-rounded btn-sm fw-bold" href="{{ route('feats.editFeat',$feat->id) }}">Edit</a></td>
                <td><a class="btn d-grid gap-2 btn-danger btn-rounded btn-sm fw-bold" href="{{ route('feats.deleteFeat',$feat->id) }}" onclick="return confirm('Are you sure you want to delete {{$feat->feat_name}}? \nThis action is permananet and non-reversable.')">Delete</a></td>
            </tr>
        @endforeach
    </tbody>
</table>
